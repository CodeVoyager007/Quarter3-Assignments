...
class FreeCourses:
    ...
    def filter_courses(self, search: str):
        if not search:
            return self.FREE_COURSES
        return [
            course for course in self.FREE_COURSES
            if search in course["title"].lower() or search in course["description"].lower() or search in course["provider"].lower()
        ]

    def display_course(self, course: dict):
    ...
    def render(self):
        ...
        filtered_courses = self.filter_courses(search)
